...
const findDiscussions = (id: string) => //This id is either the id of channel or the id of the user to dm
{
  const context:string = "dm";
  if (context === "dm")
  {

    const discussion_data =  findUserContacts(id) 
    if (!discussion_data)
      return undefined
    return ({
...
function DiscussionPanel({
  ...
  const panel = findDiscussions (DiscussionPanel.partner_id ? DiscussionPanel.partner_id : DiscussionPanel.id);
  ...
  return (
    <>
      ...
      >
        <Avatar src={panel.avatar} avatarToRight={false} status="online" />
