...
export class channelPermission implements CanActivate {
  ...
  canActivate(context: ExecutionContext): boolean | Promise<boolean> {
    ...
    if (context.getType() == 'ws') {
      ...
      if (
        context.getHandler().name === 'handleChannelUnBan' ||
        context.getHandler().name === 'handleChannelUnMute'
      )
        return this.verifyUnBanUnMutePermission(user_id, data, subscribedRoles);
      if (
        context.getHandler().name === 'upgradeUserToAdmin' ||
      ...
      if (context.getHandler().name === 'handleGradeUserTOwner')
        return this.verifyPermissionGradeUserTOwner(
          user_id,
          data,
          subscribedRoles,
        );
  ...
  async verifyMuteBanPermission(
    ...
    if (
      memberToAct.is_banned ||
      targetedMember.is_banned ||
      targetedMember.is_muted
    )
      return false;
    if (
...
export class allowJoinGuard implements CanActivate {
  ...
  async allowJoiningBySocket(
    user_id: string,
    channel_id: string,
  ): Promise<boolean> {
    return (await this.chatCrud.getMemeberShip(user_id, channel_id)) != null;
  }

  async allowJoining(
    user_id: string,
    joinRequest: channelReqDto,
  ): Promise<boolean> {
    ...
    if (user_membership == null) {
      if (
        ...
        (!joinRequest.password ||
          !(await this.chatCrud.comparePasswords(
            joinRequest.password,
...
export class userRoomSubscriptionGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    if (context.getType() == 'http') {
      ...
      if (context.getHandler().name == 'findRoomMessages') {
        ...
        const userIsSubscribed = await this.chatCrud.isUserInRoom(
          user_id,
          room_id,
        );
        return (
          userIsSubscribed.isInDMTable || userIsSubscribed.isInMembershipTable
        );
      }
      ...
      const user_id = extractUserIdFromCookies(
        context.switchToWs().getClient(),
      );
      if (context.getClass() == dmGateway)
        if (
          await this.verfiyDirectMessagingSubscription(
            context,
            packet_data,
            user_id,
          )
          ...
          return (
            (await this.chatCrud.getMemeberShip(user_id, channel_id)) !== null
          );
        }
    }
  ...
  async verfiyDirectMessagingSubscription(
    ...
    if (context.getHandler().name == 'handleSendMesDm') {
      return (
        (await this.chatCrud.checkUserInDm(user_id, packet_data.dm_id)) != null
      );
    } else if (context.getHandler().name == 'handleMarkMsgAsRead')
      return (
        (await this.chatCrud.checkUserInDm(user_id, packet_data._id)) != null
      );
      ...
      return (
        (await this.chatCrud.checkUserInDm(user_id, (await dm).id)) != null
      );
    } else if (context.getHandler().name == 'handleJoinDm') {
      return (await this.chatCrud.checkUserInDm(user_id, packet_data)) != null;
    }
  }
}
