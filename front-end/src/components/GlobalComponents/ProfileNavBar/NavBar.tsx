...
function NavBar() {
  ...
  const [userFriend, setuserFriend] = useState<
    { id: number; userFriend: string; status: string }[]
  >([]);
  ...
  const [tablenotification, settablenotification] = useState<
    { user_id: string; type: string; recipient: string }[]
  >([]);
  ...
  useEffect(() => {
    if (searchQuery !== "") {
      const filterFriends = userFriend.filter(
        (friend) =>
  ...
  return (
    <>
      <div className={NavBarCSS.nav}>
                ...
                {updateFriend.map((user) => (
                  <div className={NavBarCSS.search_user} key={user?.id}>
                    <div className={NavBarCSS.search_user_div}>
                      ...
                      <FontAwesomeIcon
                        icon={faUserPlus}
                        onClick={() => handleclickButtom(user.id)}
                      />
                    </div>
                  </div>
                ))}
        ...
        <div className={NavBarCSS.nav_left}>
          <div className={NavBarCSS.nav_icon}>
            ...
            />
            {notificationrequest && (
              <div className={NavBarCSS.notification_request}>
                <span></span>
              </div>
            )}
            {notification && (
                  ...
                  {tablenotification.map((request) => {
                    {
                      if (request.type === "follow") {
                        return (
                          <div
                            className={NavBarCSS.click_icons_friend_request}
                            key={request.user_id}
                          >
                            <div
                              className={
                                NavBarCSS.click_icons_friend_request_msg
                              }
                            >
                              <img src="../images.png" alt="Photo" />
                              <div>
                                <p id="notification-nameuser">
                                  {request.user_id}
                                </p>
                                <p> send you a friend follow request.</p>
                              </div>
                            </div>
                            <div
                              className={
                                NavBarCSS.click_icons_friend_request_button
                              }
                            >
                              <button
                                onClick={() =>
                                  socket?.emit("reponserequest", "accept")
                                }
                              >
                    ...
                    }
                    {
                      if (request.type === "game") {
                        return (
                          <div
                            className={NavBarCSS.click_icons_game_request}
                            key={request.user_id}
                          >
                            <img src="../user2.jpeg" alt="Photo" />
                            <div>
                              ...
                              <p> send you invitation to play with him</p>
                            </div>
                          </div>
                        );
                      }
      ...
      {settingindex && !closeindex && (
        <Setting handleSettingData={handlecloseindex}></Setting>
      )}
    </>
  );
}
