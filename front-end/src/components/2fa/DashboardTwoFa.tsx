  ...
  async function HandleActivate(event: any) {
    ...
    try {
      ...
      const response = await axios.post(
        `${process.env.NEXT_PUBLIC_BACKEND_SERV}/2fa/activate`, data,
        {
          headers: {
            Authorization: `Bearer ${jwtToken}`,
          },
    ...
    <div className="fixed inset-0 flex items-center justify-center z-50">
      ...
      <div className="bg-[#E4E7FF] rounded shadow-lg w-[95vw] h-[90vh] flex flex-col   p-[20px] box-border overflow-scroll">
        ...
        <div className="w-full h-[97%] flex flex-col">
          ...
          <div className="flex flex-col justify-evenly items-center md:flex-row  mb-[15px]">
          ...
          <form className="flex flex-col justify-around items-center ">
            ...
            <div className="flex justify-evenly w-full  mb-[20px] gap-24">
              <button
                className={`${mono.className} font-semibold rounded-xl text-sm border-none bg-[#862323] text-white px-[18px] py-[8px] card-shadow hover:cursor-pointer hover:bg-white hover:text-[#0D0149]`}
              >
                Desactivate
              </button>
              <button
                onClick={HandleActivate}
                className={`${mono.className} font-semibold rounded-xl text-sm border-none bg-[#23834e] text-white px-[18px] py-[8px] card-shadow hover:cursor-pointer hover:bg-white hover:text-[#0D0149]`}
              >
                Activate
              </button>
            </div>
