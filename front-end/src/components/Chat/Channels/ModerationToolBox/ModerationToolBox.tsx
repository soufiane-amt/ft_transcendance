...
export const getUserRole = (
  ...
  if (channelData && channelData?.channelAdmins.includes(user_id))
    return "Admin";
  else if (user_id === channelData?.channelOwner) return "Owner";
  else return "Member";
};
...
export function ModerationToolBox({
  selectedChannel,
  channelData,
}: ModerationToolBoxProps) {
  const currentUser = useSessionUser();
  const userContacts = useUserContacts();
  const currentUserIsModerator: string = getUserRole(
    currentUser.id,
    channelData
  );

  const sortedChannelUsers = channelData?.channelUsers.sort(
    (a: string, b: string) => {
      // Compare user roles
      ...
      const roleComparison =
        roleOrder[getUserRole(a, channelData)] -
        roleOrder[getUserRole(b, channelData)];
          ...
          <div className={style.user_moderation_part}>
            <div className={style.users_cards}>
              {sortedChannelUsers?.map((user, index) => {
                let userData;
                if (user !== currentUser.id) userData = userContacts.get(user);
                else
                  userData = {
                  ...
                  userData && (
                    <UserModerationCard
                      ...
                      targetedUser={{
                        ...
                        role: userRole,
                        isBanned:
                          channelData.channelBans.includes(user) === true,
                        isMuted:
                          channelData.channelMutes.includes(user) === true,
                      }}
                    />
                  )
                );
