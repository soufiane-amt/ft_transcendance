...
export function MuteProvider({ children, currentRoute }: MuteProviderProps) {
  ...
  useEffect(() => {
    async function fetchDataAsync() {
      const userMuteRooms: Mute[] = await fetchDataFromApi(
        `${process.env.NEXT_PUBLIC_BACKEND_SERV}/chat/${currentRoute}/MuteRooms`
      );
  ...
  function unMuteUser(roomId: string) {
    setMuteRooms((prevMuteRooms) => {
      const updatedMuteRooms = prevMuteRooms.filter(
  ...
  return (
    <MuteContext.Provider value={contextValue}>{children}</MuteContext.Provider>
  );
}
