...
export function BanProvider({ children, currentRoute }: BanProviderProps) {
  ...
  useEffect(() => {
    async function fetchDataAsync() {
      const userBannedRooms: Ban[] = await fetchDataFromApi(
        `${process.env.NEXT_PUBLIC_BACKEND_SERV}/chat/${currentRoute}/bannedRooms`
      );
  ...
  function banUser(room_id: string, blocker_id: string) {
    ...
    setBannedRooms((prevBannedRooms) => {
      ...
      if (room) return [...prevBannedRooms];
  ...
  function unbanUser(roomId: string, userId: string) {
    setBannedRooms((prevBannedRooms) => {
      ...
      if (room?.blocker_id !== userId) return prevBannedRooms;
      const updatedBannedRooms = prevBannedRooms.filter(
  ...
  return (
    <BanContext.Provider value={contextValue}>{children}</BanContext.Provider>
  );
}
